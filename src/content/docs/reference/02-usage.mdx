...
See the section [Translate Module API](#translate-module-api) for details.
...
use [AoT compilation](https://angular.io/docs/ts/latest/cookbook/aot-compiler.html) or [Ionic](http://ionic.io/).
...
[`setDefaultLang()`](#set-default-lang) sets the default language. The default language is used
...
[`use()`](#use) sets the current language for the application. If the selected language is not yet
...
You can also define your translations manually with [`setTranslation()`](#set-translation).
...
We highly recommend using [BabelEdit](/babeledit) to manage your translations. It allows you to
...
You might ask yourself, why [`get()`](#get) is using a subscription to receive the translation.
This is because your language file might not be loaded when the [get()](#get) function is called.
The update is triggered when the loading is complete.

You can also use [`stream()`](#stream) which is similar to `get()` but also fires when the language is changed.

There is also [`instant()`](#instant) &mdash; a synchronous method that returns the translation value
immediately. This only works if the translation file is already loaded &mdash; or the translations
are set using  [`setTranslation()`](#set-translation).
