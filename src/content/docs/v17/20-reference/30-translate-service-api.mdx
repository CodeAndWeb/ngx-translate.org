...
description: Complete TranslateService API reference for ngx-translate. Learn
  all methods for Angular translation including get(), stream(), instant(),
  use(), and more with examples.
slug: v17/reference/translate-service-api
---
...
The translated value(s), returned synchronously:

* If a single key is provided, returns the translated message as `Translation`.
* If an array of keys is provided, returns an object `{ [key: string]: Translation }` mapping each key to its translated string.
