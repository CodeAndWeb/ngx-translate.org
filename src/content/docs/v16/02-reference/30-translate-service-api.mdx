...
- [getTranslation](#gettranslation) - Load and compile translations
...
:::caution
After calling `resetLang`, you will need to call [`use()`](#use),
[`reloadLang()`](#reloadlang), or [`getTranslation()`](#gettranslation) again to
...
Changes the currently active language to the specified language code. This method triggers the
loader to retrieve the translations for the new language if they have not been loaded yet.
...
If the translations for the specified language are not already loaded, it will use the
configured loader to fetch them. This method also updates the list of available languages,
which can be accessed using [`getLangs()`](#getlangs).
...
- **Avoid Rapid Successive Calls**: Avoid making multiple calls to `use()` in quick succession.
If translations are loaded from a server, subsequent calls may complete before previous ones,
potentially leaving the `TranslateService` in an inconsistent state.
Wait for the returned `Observable to resolve before calling it again.
- **Impact on Existing Subscriptions**: Components or services that are subscribed to
translation observables (e.g., via [`get()`](#get) or [`stream()`](#stream) will receive updates when the
language changes.
- **Fallback Language**: If a translation key is missing in the new current language,
the service will attempt to retrieve it from the default language set via [`setDefaultLang()`](#setdefaultlang).
- **Available Languages**: The specified language will be added to the list of available languages
accessible via [`getLangs()`](#getlangs) if it's not already included.
