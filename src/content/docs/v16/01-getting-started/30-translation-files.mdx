...
slug: v16/getting-started/translation-files
---
...
**@codeandweb/ngx-translate** does not come with a loader. The reason for this is to keep the
module as small as possible and to make it adaptable to your needs.

However, there's an official plugin called **`@codeandweb/http-loader`** that implements
a dynamic loader.

## Static Translations via `import`

The easy way to set translation data is using `import` and [`setTranslation()`](/v16/reference/translate-service-api#settranslation).
...
<PackageManagers pkg={"@codeandweb/http-loader"} />
...
```ts {2-8,13-20} title="app.config.ts"
...
by implementing your own [loader](/v16/reference/translate-loader-api) or using one of the
[plugins](/v16/resources/plugins).
