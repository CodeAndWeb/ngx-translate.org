...
<ng-container *translateBlock="let t">
  <h1>{{ t('app.title') }}</h1>
  <p>{{ t('app.hello', { name: user.name }) }}</p>
</ng-container>
~~~
...
**When to choose which form.** `*translateBlock` is the cleanest fit when you have
...
[Concepts → Which API should I use?](/reference/concepts/#which-api-should-i-use).

#### Side-by-side: pipe vs `*translateBlock`
...
<div class="card">
  ...
  <p>{{ 'card.body' | translate:{ name: user.name } }}</p>
...
<ng-container *translateBlock="let t">
  <div class="card">
    <h1>{{ t('card.title') }}</h1>
    <p>{{ t('card.body', { name: user.name }) }}</p>
    <button>{{ t('common.save') }}</button>
    <button>{{ t('common.cancel') }}</button>
  </div>
</ng-container>
...
Both produce identical output and identical reactivity. Pick `*translateBlock`
...
`fallbackLang` is the symmetric reactive Signal for the fallback language, with the same
...
use [`getCurrentLang()`](/reference/translate-service-api/#getcurrentlang) or
[`getFallbackLang()`](/reference/translate-service-api/#getfallbacklang).
...
In Angular templates the pipe (`|`) has lower precedence than `+`, so
`'languages.' + language | translate` parses as
`('languages.' + language) | translate`. The behavior is the same with or
without the parentheses, but writing them explicitly is easier to read.
:::
