...
automatically, so the UI updates whenever the underlying signal changes — no subscriptions,
no manual change detection. If you're new to Signals, the [official Angular guide](https://angular.dev/guide/signals)
...
block up to date when the language changes or translations are reloaded. A full cookbook
of patterns will follow — for now this minimal example is enough to migrate off
content-as-key.
...
component class field). It returns a `Signal<Translation>` that updates automatically on
language changes or translation reloads — no subscription, no cleanup:
