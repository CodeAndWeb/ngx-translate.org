...
| TS: `'extend' does not exist in type 'ChildTranslateServiceConfig'` | Move to `provideChildTranslateService()` (where it is no longer needed — connected children are the default). |
...
**Silent behavior change:** Existing `setTranslation(lang, translations)` calls now replace instead of merge. Pass `true` as the third argument if you depended on the v17 merge behavior.
...
- Skim the [v18 release notes](/#whats-new-in-v18) for new features (signal-driven pipe/directive, `*translateBlock`, per-call `lang`, `translate()` standalone function, hierarchical services).
