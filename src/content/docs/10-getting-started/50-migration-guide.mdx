...
|------------|---------------|
| `"@ngx-translate/core": "^17` | Pre-flight: Angular peer dep is now `^18.0.0`. See [Angular 16 → 18 peer dependency bump](#angular-16--18-peer-dependency-bump--impact-high-pre-flight). |
...
|------------------|-----|
| `npm ERR! ERESOLVE could not resolve` … `peer @angular/core@"^18.0.0"` | Upgrade Angular first. See [Angular 16 → 18 peer dependency bump](#angular-16--18-peer-dependency-bump--impact-high-pre-flight). |
...
v18 raises the minimum Angular peer dependency from `^16.0.0` to `^18.0.0`.
Projects on Angular 16 or 17 must upgrade Angular first (`ng update @angular/core@18`)
before installing `@ngx-translate/core@18`. The library uses signal-based reactivity
and Angular DI features that landed in Angular 18.

If you're on Angular 18 already, no action needed.
...
See [`isLoading`](/reference/translate-service-api/#isloading) and the
[loading-indicator recipe](/recipes/show-language-loading-indicator/) for details.
