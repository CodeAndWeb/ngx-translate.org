...
`@ngx-translate/http-loader` is optional. It allows loading translation files on demand from the server.
...
In your components, import `TranslatePipe` and `TranslateDirective` to your `imports`. You can access the
...
fallback language using `fallbackLang`.
...
[`setFallbackLang()`](/reference/translate-service-api/#setfallbacklang) sets the fallback language. The fallback language is used
when no translation is found in the currently selected language.
You can also do this at application startup using the `TranslateModule.forRoot({fallbackLang:'en'})`.
...
:::note
**Terminology Change in v17**

In NGX-Translate v17, the terminology has been standardized to use "fallback language" instead of "default language" for clarity. The fallback language is used when a translation is missing in the current language.
...
The `setFallbackLang()` method replaces the deprecated `setDefaultLang()` method, and the `fallbackLang` configuration option replaces the deprecated `defaultLanguage` option.
:::
