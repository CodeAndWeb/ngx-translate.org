...
:::caution
This method is deprecated in v17. Use [`getFallbackLang()`](#getfallbacklang) instead.
:::
...
The `getDefaultLang()` method retrieves the fallback language that has been set
using the [`setFallbackLang()`](#setfallbacklang) method. This is the language code that the `TranslateService`
uses as a fallback when a translation key is missing in the current language.
...
~~~ts
setDefaultLang(lang: string): Observable<InterpolatableTranslationObject>
~~~

Sets the fallback language to be used when a translation is missing in the current language.

:::caution
This method is deprecated in v17. Use [`setFallbackLang()`](#setfallbacklang) instead.
:::
...
The `setDefaultLang()` method specifies the fallback language that the `TranslateService`
will use to look up translations when a key is not found in the current language.
...
~~~ts
translateService.setDefaultLang("en").subscribe((translations) => {
    console.log('Fallback language set to English');
});
~~~
...
| Property                                     | Description                                               |
|---------------------------------------------------------|-----------------------------------------------------------|
| `langs` <Badge text="Deprecated" variant="danger" />                                  | List of languages. Use `getLangs()` instead.      <br /> `readonly string[]`                         |
| `currentLang` <Badge text="Deprecated" variant="danger" />                                  | The language currently in use. Access the current language through this property.     <br /> `string`                         |
| `defaultLang` <Badge text="Deprecated" variant="danger" />                                | The fallback language. Use `getFallbackLang()` instead.   <br /> `string | null`                           |
| `onDefaultLangChange` <Badge text="Deprecated" variant="danger" />  | Observable that emits when the fallback language changes. Use `onFallbackLangChange` instead. <br /> `Observable<DefaultLangChangeEvent>`|
...
:::caution
This observable is deprecated in v17. Use [`onFallbackLangChange`](#onfallbacklangchange) instead.
:::
